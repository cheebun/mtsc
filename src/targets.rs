...
/// formula -- see 2026-09-07 real-VM test in `targets::tests` and `check_match`/
...
/// Cross-checked against the plain-division formula for every real `mbr_val` and a large
/// random sample by the tests below.
...
/// `keys.toml` entries and decode their SOFTWARE ID -- `load_targets`'s `Target.need_hi`
/// retains the full width but already incorporates a specific mix. Callers checking
/// feasibility across all 2048 `mbr_val` values need the raw (tv_lo, tv_hi) pair instead.
///
...
"#,
    ...
    #[test]
    fn test_load_from_file_sid_only_and_optional_signatures() {
        let path = write_temp_keys_toml(
            "optional_signatures",
            r#"[[key]]
software_id = "TTTT-TTTT" # SID-only entries were accepted by the old scanner
level = 6
identity_note = "synthetic metadata"
[[key]]
software_id = "NTTT-TTTT"
signature_hex = ""
[[key]]
software_id = "0TTT-TTTT"
signature_hex = "AA"
"#,
        );
        let entries = load_from_file(&path).expect("legacy entries should parse");
        let _ = fs::remove_file(path);
        assert_eq!(entries.len(), 3);
        assert_eq!(entries[0].software_id, "TTTT-TTTT");
        assert!(entries[0].signature_hex.is_empty());
        assert!(entries[1].signature_hex.is_empty());
        assert_eq!(entries[2].signature_hex, "AA");
        let targets = entries_to_targets(&entries, mbr_mix());
        assert_eq!(targets.len(), 3);
        assert!(targets[0].signature_hex.is_empty());
        assert_eq!(targets[2].signature_hex, "AA");
    }

    #[test]
    fn test_load_from_file_inline_sid_only_entry() {
        let path = write_temp_keys_toml(
            "inline_sid_only",
            r#"key = [{ software_id = "TTTT-TTTT", note = "synthetic" }]"#,
        );
        let entries = load_from_file(&path).expect("inline SID-only entry should parse");
        let _ = fs::remove_file(path);
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].software_id, "TTTT-TTTT");
        assert!(entries[0].signature_hex.is_empty());
    ...
    /// `software-id: ZTBI-ENJL`, whose `tv_hi=0x144` is inside `256..512`. Both the old
    /// masked rule and the full-width rule accept this actual SID; only the full-width
    /// rule rejects the unreachable `MGT2-L23Y` target predicted by the old sweep.
    #[test]
    fn test_entries_to_targets_matches_real_hardware_ztbi_enjl() {
        ...
        assert_eq!(software_id::decode("ZTBI-ENJL").unwrap() >> 32, 0x144);
        assert_eq!(sid_lo, target.need_lo);
        assert_eq!((sid_hi as u32) | 0x100, target.need_hi);
    }

    #[test]
    fn test_entries_to_targets_preserves_unreachable_high_bits() {
        let (mix_lo, mix_hi) = mbr_mix();
        let tv_lo = 0x1234_5678u32;
        for tv_hi in [0u32, 0x44, 0xFF, 0x200] {
            let entries = [KeyEntry {
                software_id: software_id::encode(((tv_hi as u64) << 32) | tv_lo as u64),
                signature_hex: String::new(),
            }];
            let target = &entries_to_targets(&entries, (mix_lo, mix_hi))[0];
            let sid_lo = tv_lo ^ mix_lo;
            let sid_hi = (tv_hi ^ mix_hi) as u8;
            // The old low-byte comparison would accept this synthetic hash; the full
            // comparison must retain the bits making this target structurally impossible.
            assert_eq!(sid_lo, target.need_lo);
            assert_eq!(sid_hi, target.need_hi as u8);
            assert_eq!(target.need_hi, tv_hi ^ mix_hi);
            assert_ne!((sid_hi as u32) | 0x100, target.need_hi);
        }
    }
    ...
    fn test_marker_from_identity_low_11_bits_match_mix_from_identity() {
        ...
        for identity in [
            ...
            let (mix_lo, mix_hi) = mix_from_identity(&identity);
            let expected_mix = ((raw16 as u64) & 0x7FF) * MIX_MULTIPLIER;
            assert_eq!(
                (mix_lo, mix_hi),
