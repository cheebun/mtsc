...
mod tests {
    ...
    /// Gate every required feature, without imposing AVX or AVX2 on SHA-NI tests.
    fn supported() -> bool {
        let supported = is_x86_feature_detected!("sha")
            && is_x86_feature_detected!("ssse3")
            && is_x86_feature_detected!("sse4.1");
        if !supported {
            eprintln!("SKIP: SHA + SSSE3 + SSE4.1 not supported on this CPU");
        }
        supported
    }
    ...
    /// Compare every independent buffer with both existing scalar implementations.
    fn check_batch<const N: usize>(inputs: &[[u8; 40]; N]) -> [(u32, u8); N] {
        let mut outputs = [(0, 0); N];
        // SAFETY: Tests call this only after feature detection, for N = 1, 2, 4,
        // with exact lengths and a shared tail matching the precomputed words.
        unsafe { hash_batch::<N>(inputs, &constant_words(&inputs[0]), &mut outputs) };
        for (lane, (input, &output)) in inputs.iter().zip(&outputs).enumerate() {
            assert_eq!(output, crate::sha256::hash_40(input), "N={N}, lane {lane}");
            assert_eq!(
                output,
                crate::sha256_scalar::hash_40(input),
                "N={N}, lane {lane}"
            );
        }
        outputs
    ...
    /// Check the known 6G value in the first buffer, with distinct neighboring inputs.
    fn check_6g_known<const N: usize>() {
        let inputs = std::array::from_fn(|lane| {
            let mut input = [0x20; 40];
            input[..20].copy_from_slice(format!("{:020}", lane + 1).as_bytes());
            input[20..36].copy_from_slice(b"VMware Virtual I");
            input[36..40].copy_from_slice(&0x1800u32.to_le_bytes());
            input
        });
        assert_eq!(check_batch::<N>(&inputs)[0], (0x0B49EC2E, 0x35));
    }

    /// Exercise decimal serials with the same common model/sector tail.
    fn check_random_serials<const N: usize>() {
        ...
        for _ in 0..128 {
            ...
            check_batch::<N>(&inputs);
        }
    }

    /// Vary common tails and binary prefixes to stress scheduling and endianness.
    fn check_varied_uniform_tails<const N: usize>() {
        let mut random = 0xa54f_f53a_5f1d_36f1;
        for case in 0..128 {
    ...
    /// Check unaligned message loads and writes restricted to the output subslice.
    fn check_sliced_unaligned_buffers<const N: usize>() {
        #[repr(align(16))]
        struct Storage([u8; 1 + 4 * 40]);
        let mut storage = Storage([0; 1 + 4 * 40]);
        // SAFETY: Tests instantiate N <= 4; [u8; 40] has alignment one, and
        // all N complete messages fit in the storage following the offset byte.
        let inputs = unsafe { &mut *storage.0.as_mut_ptr().add(1).cast::<[[u8; 40]; N]>() };
        ...
        // SAFETY: Features are detected, N is supported, and both slices have N lanes.
        unsafe { hash_batch::<N>(inputs, &constant_words(&inputs[0]), &mut outputs[1..=N]) };
        assert_eq!(outputs[0], sentinel);
        assert!(outputs[N + 1..].iter().all(|&output| output == sentinel));
        for (input, &output) in inputs.iter().zip(&outputs[1..=N]) {
            assert_eq!(output, crate::sha256::hash_40(input));
            assert_eq!(output, crate::sha256_scalar::hash_40(input));
        }
    }

    /// Exercise every supported compile-time buffer count as a separate test.
    macro_rules! test_buffer_count {
        ($name:ident, $n:literal) => {
            #[doc = concat!("Validate the ", stringify!($n), "-buffer SHA-NI specialization.")]
            #[test]
            fn $name() {
                if !supported() {
                    return;
                ...
                check_6g_known::<$n>();
                check_random_serials::<$n>();
                check_varied_uniform_tails::<$n>();
                check_sliced_unaligned_buffers::<$n>();
            }
        };
    }

    test_buffer_count!(test_shani_x1, 1);
    test_buffer_count!(test_shani_x2, 2);
    test_buffer_count!(test_shani_x4, 4);
}
