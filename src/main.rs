...
use mtsc::sha256;
#[cfg(test)]
use mtsc::sha256_backend::precompute_constant_words;
...
mod tests {
    ...
    fn make_test_ctx(targets: Vec<targets::Target>) -> SearchContext {
        let (mix_lo, mix_hi) = targets::mbr_mix();
        SearchContext {
            model_bytes: *b"VMware Virtual I",
            sv_bytes: 0x1800u32.to_le_bytes(),
    ...
    fn make_fake_target() -> targets::Target {
        targets::Target {
            ...
            name: {
                let (lo, hi) = targets::mbr_mix();
                compute_software_id(0x0B49EC2E, 0x35, lo, hi)
            },
    ...
    fn test_check_match_stops_at_target_count() {
        let mut ctx = make_test_ctx(vec![make_fake_target()]);
    ...
    /// Exercise the real loop with a timeout, including no-hit exhaustion cases.
    fn run_search_test(
        engine: HashEngine,
        tid: usize,
        threads: usize,
        ...
        let worker = thread::spawn(move || {
            search_batch(tid, threads, start, &worker_ctx, engine);
            let _ = done_tx.send(());
        });
        ...
        worker.join().unwrap();
        assert!(done.is_ok(), "{engine}: search did not finish");
        ctx
    }

    /// Independent index encoding, without the loop's incremental counter/pad helpers.
    fn reference_serial(mut index: u64, alphabet: &[u8], pad: PadPosition) -> [u8; SERIAL_LEN] {
        let mut digits = Vec::new();
        loop {
            digits.push(alphabet[(index % alphabet.len() as u64) as usize]);
        ...
        digits.reverse();
        let mut bytes = match pad {
            PadPosition::Start => [alphabet[0]; SERIAL_LEN],
            PadPosition::End => [SPACE_PADDING; SERIAL_LEN],
        };
        let offset = if pad == PadPosition::Start {
            SERIAL_LEN - digits.len()
        } else {
            0
        };
        bytes[offset..offset + digits.len()].copy_from_slice(&digits);
        bytes
    }

    fn plant_target(ctx: &mut SearchContext, index: u64, sweep: bool) {
        let serial = reference_serial(index, &ctx.alphabet, ctx.pad);
        let (lo, hi) = sha256::hash_40(&build_input_buf(&serial, &ctx.model_bytes, &ctx.sv_bytes));
        if sweep {
            let mix = 555 * targets::MIX_MULTIPLIER;
            let name = compute_software_id(lo, hi, mix as u32, (mix >> 32) as u32);
            let tv = software_id::decode(&name).unwrap();
            ctx.raw_targets = Some(Arc::new(vec![targets::RawTarget {
                name,
                tv_lo: tv as u32,
                tv_hi: (tv >> 32) as u32,
                signature_hex: String::new(),
            }]));
            // Explicit missing path avoids loading a user's working-directory table.
            static TABLE: std::sync::OnceLock<Arc<mbr_table::MbrTable>> =
                std::sync::OnceLock::new();
            ctx.mbr_table = Some(Arc::clone(TABLE.get_or_init(|| {
                Arc::new(mbr_table::MbrTable::load(Some(
                    "/nonexistent/mtsc-unit-test/mbr-table.toml",
                )))
            })));
            ctx.targets = Arc::new(Vec::new());
            ctx.mix_lo = 0;
            ctx.mix_hi = 0;
        } else {
            ctx.targets = Arc::new(vec![targets::Target {
                need_lo: lo,
                need_hi: (hi as u32) | 0x100,
                name: compute_software_id(lo, hi, ctx.mix_lo, ctx.mix_hi),
                signature_hex: String::new(),
            }]);
        }
    }
    ...
    fn test_search_batch_all_backends_and_thread_offsets() {
        for engine in HashEngine::supported() {
            for alphabet in [
                ...
                for pad in [PadPosition::Start, PadPosition::End] {
                    for sweep in [false, true] {
                        for threads in [1, 3, 257] {
                            for tid in [0, threads - 1] {
                                for start in [9, 35, 9_999_999_990, u64::MAX - 2] {
                                    for batch_index in [0, 3] {
                                        for lane in [0, engine.batch_size() - 1] {
                                            let offset = ((batch_index * threads + tid)
                                                * engine.batch_size()
                                                + lane)
                                                as u64;
                                            let index = start.wrapping_add(offset);
                                            let mut ctx = make_test_ctx(vec![]);
                                            ctx.alphabet = alphabet.to_vec();
                                            ctx.is_default_alphabet = alphabet == b"0123456789";
                                            ctx.pad = pad;
                                            ctx.max_collisions = 1;
                                            plant_target(&mut ctx, index, sweep);
                                            let ctx =
                                                run_search_test(engine, tid, threads, start, ctx);
                                            assert_eq!(
    ...
    #[test]
    fn test_small_alphabet_exhaustion_and_partial_batches() {
        for engine in HashEngine::supported() {
            for pad in [PadPosition::Start, PadPosition::End] {
                for sweep in [false, true] {
                    let limit = candidate_space_limit(2).unwrap();
                    for needle in [0, limit - 1] {
                        let mut ctx = make_test_ctx(vec![]);
                        ctx.alphabet = b"AB".to_vec();
                        ctx.is_default_alphabet = false;
                        ctx.pad = pad;
                        // Unlimited: exhaust naturally, rather than hide duplicate tail hits.
                        plant_target(&mut ctx, needle, sweep);
                        let ctx = run_search_test(engine, 0, 1, limit - 3, ctx);
                        assert_eq!(
                            ctx.found_count.load(Ordering::Relaxed),
                            usize::from(needle != 0),
                            "{engine}"
                    ...
                    ctx.is_default_alphabet = false;
                    ctx.pad = pad;
                    plant_target(&mut ctx, 0, sweep);
                    let ctx = run_search_test(engine, 2, 3, limit - 1, ctx);
                    assert_eq!(ctx.found_count.load(Ordering::Relaxed), 0);
                }
            }
        }
    ...
    #[test]
    fn test_search_hit_verification_rejects_mismatch() {
        let ctx = make_test_ctx(vec![make_fake_target()]);
        let name = &ctx.targets[0].name;
        let mix = (ctx.mix_lo, ctx.mix_hi);
        assert!(verify_search_hit(1, (0x0B49EC2E, 0x35), mix, name, &ctx).is_ok());
        assert!(verify_search_hit(2, (0x0B49EC2E, 0x35), mix, name, &ctx).is_err());
        assert!(verify_search_hit(1, (0, 0), mix, name, &ctx).is_err());
        assert!(verify_search_hit(1, (0x0B49EC2E, 0x35), (0, 0), name, &ctx).is_err());
        assert!(verify_search_hit(1, (0x0B49EC2E, 0x35), mix, "invalid", &ctx).is_err());
    }

    #[test]
    fn test_full_width_target_rejects_same_low_byte() {
        for need_hi in [0x35, 0x235, 0xFFFF_FF35] {
            let mut target = make_fake_target();
            target.need_hi = need_hi;
            let ctx = make_test_ctx(vec![target]);
            check_match(1, 0x0B49EC2E, 0x35, &ctx);
            assert_eq!(ctx.found_count.load(Ordering::Relaxed), 0);
        }
    }
    ...
    #[test]
    fn test_candidate_limits_and_resume_bounds() {
        assert_eq!(candidate_space_limit(2), Some(1 << 20));
        assert_eq!(candidate_space_limit(9), Some(9u64.pow(20)));
        assert_eq!(candidate_space_limit(10), None);
        assert_eq!(candidate_space_limit(62), None);
        assert_eq!(
            resolve_start_serial(1, candidate_space_limit(2)),
            Ok(1_000_000)
        );
        assert!(resolve_start_serial(2, candidate_space_limit(2)).is_err());
        let max = u64::MAX / 1_000_000;
        assert_eq!(resolve_start_serial(max, None), Ok(max * 1_000_000));
        assert!(resolve_start_serial(max + 1, None).is_err());
    }

    #[test]
    fn test_scsi_without_size_requires_literal_model() {
        let (_, label) = resolve_disk_size(None, SizeUnit::G, BusType::Scsi);
        assert!(resolve_model(None, None, &label).is_err());
        assert_eq!(
            resolve_model(Some("DiskModel".into()), None, &label).unwrap(),
            "DiskModel"
        );
        assert_eq!(resolve_model(None, Some(1), "1G").unwrap(), "ROS1G");
    }
