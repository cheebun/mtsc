...
mod tests {
    ...
    #[test]
    fn every_supported_backend_matches_both_scalars() {
        let mut seed = 0x9182_7364_abcd_ef01_u64;
        let mut next = || {
            seed ^= seed << 13;
            seed ^= seed >> 7;
            seed ^= seed << 17;
        ...
        for engine in HashEngine::supported() {
            engine.self_check().unwrap();
            for _ in 0..128 {
                let model = std::array::from_fn(|_| next());
                let sv = std::array::from_fn(|_| next());
                let mut batch = HashBatch::new(engine, &model, &sv);
                for lane in 0..batch.len() {
                    *batch.serial_mut(lane) = std::array::from_fn(|_| next());
                }
                batch.hash();
                for (input, output) in batch.inputs.iter().zip(batch.outputs()) {
                    assert_eq!(*output, crate::sha256::hash_40(input), "{engine}");
                    assert_eq!(*output, crate::sha256_scalar::hash_40(input), "{engine}");
                }
            }
        }
    }
    ...
    #[test]
    fn backend_sizes_and_reused_batches() {
        for engine in HashEngine::supported() {
            let expected = match engine.backend() {
                HashBackend::Scalar => 1,
                HashBackend::Avx2 => 8,
                HashBackend::Avx512 => 16,
                HashBackend::Neon => 4,
                HashBackend::ShaNi | HashBackend::ArmSha2 => engine.batch_size(),
            };
            let mut batch = HashBatch::new(engine, &[0xff; 16], &[0x80; 4]);
            assert_eq!(batch.len(), expected);
            assert!(!batch.is_empty());
            for value in [0, 255, 1, 128] {
                for lane in 0..batch.len() {
                    *batch.serial_mut(lane) = [value; 20];
                }
                batch.hash();
                for (input, output) in batch.inputs.iter().zip(batch.outputs()) {
    ...
    #[test]
    fn calibration_rejects_zero_workers() {
        assert!(HashEngine::auto_for_threads(0).is_err());
    }

    #[test]
    fn calibration_reports_worker_panics() {
        unsafe fn failing_kernel(_: &[[u8; 40]], _: &[u32; 5], _: &mut [(u32, u8)]) {
            panic!("synthetic calibration failure");
        }
        ...
        assert_eq!(
            sample_rate(engine, 2, Duration::from_millis(1)),
            Err("SHA-256 calibration worker panicked")
        );
    }

    #[test]
    fn automatic_selection_is_supported_and_cached() {
        let first = HashEngine::auto().unwrap();
        let second = HashEngine::auto().unwrap();
        assert_eq!(
            (first.backend(), first.batch_size()),
            (second.backend(), second.batch_size())
        );
        assert!(HashEngine::supported()
            .iter()
            .any(|e| e.backend() == first.backend() && e.batch_size() == first.batch_size()));
        first.self_check().unwrap();
    }
}
